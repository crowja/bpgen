...
bpgen.o: bpgen.c bpgen.h
	$(CC) -c $(CPPFLAGS) $(CFLAGS) -o $@ bpgen.c
...
check-examples:
	...
	do \
	  ...
	  ( $(CC) -g $(CPPFLAGS) $(OTHER_INCLUDE) $(CFLAGS) $(OTHER_SOURCE) \
		-o ex/a.out $$i.c bpgen.c $(LDFLAGS) ) \
...
echeck:
	...
	do \
	  ...
	  echo "Running test $$i ..."; \
	  ( $(CC)    $(CPPFLAGS) $(OTHER_INCLUDE) $(CFLAGS) $(OTHER_SOURCE) \
		-o t/a.out $$i.c bpgen.c $(LDFLAGS_EFENCE) ) \
	  && ( LD_PRELOAD=libefence.so ./t/a.out ); \
	done 

indent: stamp
	@indent $(INDENT_FLAGS) bpgen.c
	@indent $(INDENT_FLAGS) bpgen.h
	@indent $(INDENT_FLAGS) t/test.c
	@indent $(INDENT_FLAGS) ex/ex_1.c

stamp:
	@bash stamper.bash bpgen.c
	@bash stamper.bash bpgen.h
